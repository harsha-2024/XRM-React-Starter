...
const InvoiceSchema = z.object({ number: z.string().min(1,'Required'), account: z.string().min(1,'Required'), amount: z.coerce.number().positive('Must be positive'), status: z.enum(['Draft','Sent','Paid','Overdue']) })
...
export default function InvoicesPage(){
  ...
  const { register, handleSubmit, reset, formState:{errors} } = useForm<InvoiceForm>({ resolver: zodResolver(InvoiceSchema), defaultValues: { number:'', account:'', amount:0, status:'Draft' } })

  async function refresh(){ setRows(await listInvoices()) }
  useEffect(()=>{ refresh() }, [])

  async function onSubmit(data: InvoiceForm){ if(editing){ await updateInvoice(editing.id, data) } else { await createInvoice(data) } await refresh(); setEditing(null); reset({ number:'', account:'', amount:0, status:'Draft' }) }
              ...
              <tbody>
                {rows.map(r=> (
                  <tr key={r.id}>
                    ...
                    <td style={{ padding:8 }}>
                      <IconButton size='small' onClick={()=>{ setAttInvoiceId(r.id); setAttOpen(true) }}><AttachFileIcon fontSize='small' /></IconButton>
                    </td>
                    <td style={{ padding:8 }}>
